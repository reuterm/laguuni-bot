...
const PRO_URL =
  "https://laguuniin.fi/app/themes/LevelupSkele/templates/bookly_ajax.php?service=pro_kaapeli_keilis&location=1";
const PRO_TIME_SLOTS_URL = (csrfToken, formId) =>
  `https://laguuniin.fi/wp/wp-admin/admin-ajax.php?lang=fi&action=bookly_render_time&csrf_token=${csrfToken}&form_id=${formId}`;
...
async function getCsrfToken(response) {
  ...
  const tokenObject =
    tokenMatches && tokenMatches.length > 0 ? JSON.parse(tokenMatches[0]) : {};
...
async function getFormId(sessionCookie) {
  ...
  const idObject =
    idMatches && idMatches.length > 0 ? JSON.parse(`{${idMatches[0]}}`) : {};
